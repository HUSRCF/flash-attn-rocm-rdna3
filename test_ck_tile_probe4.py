...
def run_case(batch, seqlen, nheads, headdim, dtype, causal, seed) -> Tuple[float, float, float, float, float]:
    ...
    # =========================================================================
    # 探针 1：显存对齐检查 (Pointer Alignment Probe)
    # RDNA3 (gfx1100) 的 CK kernel 极度依赖 16B/32B/256B 对齐。
    # 如果 1024 崩溃时的余数和 512->1024 时的余数不同，说明石锤是对齐 Bug。
    # =========================================================================
    if seqlen == 1024:
        align_q = q.data_ptr() % 256
        align_k = k.data_ptr() % 256
        align_v = v.data_ptr() % 256
        align_dout = dout.data_ptr() % 256
        print(f"\n[Align Probe] Seq={seqlen} | Q:{align_q}, K:{align_k}, V:{align_v}, dOut:{align_dout}")

    # =========================================================================
    # 探针 2：TLB / 工作区预热清洗 (Workspace Warmup Wash)
    # 模拟 512->1024 的“热车”过程。强制 PyTorch Caching Allocator 分配并
    # 映射 FA 内部所需的工作区（如 softmax_lse, RNG state），消除缺页异常和脏数据。
    # =========================================================================
    if seqlen == 1024:
        # 显式向显存写一点垃圾数据，强迫物理页分配
        _dummy = torch.empty(64 * 1024 * 1024, dtype=torch.uint8, device="cuda").fill_(1)
        with torch.no_grad():
            # 空跑一次，让 FA 把该申请的内部 Buffer 全申请了，该建的页表全建了
    ...
    # =========================================================================
    # 探针 3：NaN / Inf 拦截器 (NaN Trap)
    # 防止未初始化的脏寄存器导致的 NaN 顺着 BWD 蔓延，引发核异常崩溃。
    # =========================================================================
    if out_fa.isnan().any() or out_fa.isinf().any():
        print(f"\n[💥 FATAL] Seq={seqlen} 前向 FWD 结果已经包含 NaN/Inf! BWD 必炸，拦截！")

    dout *= 10
    
    # 严格确保梯度是完全干净的（洗掉 Caching Allocator 给的脏显存）
    if q.grad is not None: q.grad.zero_()
    if k.grad is not None: k.grad.zero_()
    if v.grad is not None: v.grad.zero_()
    
    # 手动为第一次 backward 分配全零梯度，防止 FA 后向 kernel 累加到未初始化的垃圾内存上
    q.retain_grad()
    k.retain_grad()
    v.retain_grad()
        ...
        for causal in causal_list:
            for headdim in dim_list:
                for seqlen in seqlens:
                    ...
                    ok = (fwd_mean <= args.fw_atol) and (bwd_mean <= args.bw_atol)
                    row = Row(
                        tag=args.tag,
                        seqlen=seqlen,
                        headdim=headdim,
