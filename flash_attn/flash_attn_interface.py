...
def _needs_gfx11_varlen_d256_deterministic_fallback(
    q: torch.Tensor, deterministic: bool
) -> bool:
    if not deterministic or q.shape[-1] != 256 or not torch.version.hip:
        return False
    arch = getattr(torch.cuda.get_device_properties(q.device), "gcnArchName", "")
    return arch.split(":", 1)[0].startswith("gfx11")


def _flash_attn_varlen_d256_deterministic_fallback(
    dout: torch.Tensor,
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    # The older CK gfx1x D256 deterministic group kernel can access beyond its
    # packed dQ workspace. The batch kernel does not use that group workspace,
    # so run each packed sequence independently until the unified CK workspace
    # is ported to this branch.
    if dropout_p != 0.0:
        raise RuntimeError(
            "gfx11 D256 deterministic varlen backward with dropout is unsupported"
        )
    with torch.cuda.device(q.device):
        if torch.cuda.is_current_stream_capturing():
            raise RuntimeError(
                "gfx11 D256 deterministic varlen backward fallback is not HIP graph "
                "capturable; use deterministic=False during graph capture"
            )
        q_offsets = cu_seqlens_q.detach().cpu().tolist()
        k_offsets = cu_seqlens_k.detach().cpu().tolist()
    if (
        len(q_offsets) < 2
        or len(q_offsets) != len(k_offsets)
        or q_offsets[0] != 0
        or k_offsets[0] != 0
        or q_offsets[-1] != q.shape[0]
        or k_offsets[-1] != k.shape[0]
        or any(start > end for start, end in zip(q_offsets[:-1], q_offsets[1:]))
        or any(start > end for start, end in zip(k_offsets[:-1], k_offsets[1:]))
    ):
        raise RuntimeError("invalid cu_seqlens for gfx11 D256 backward fallback")

    softmax_d = torch.zeros(
    ...
    for batch_index, (q_start, q_end, k_start, k_end) in enumerate(
        zip(q_offsets[:-1], q_offsets[1:], k_offsets[:-1], k_offsets[1:])
    ):
        if q_start == q_end:
            dk[k_start:k_end].zero_()
            dv[k_start:k_end].zero_()
            continue
        if k_start == k_end:
        ...
        alibi_slopes_batch = (
            alibi_slopes
            if alibi_slopes is None or alibi_slopes.ndim == 1
            else alibi_slopes[batch_index : batch_index + 1]
        )
        softmax_d_batch = flash_attn_gpu.bwd(
            dout[q_start:q_end].unsqueeze(0),
            q[q_start:q_end].unsqueeze(0),
            k[k_start:k_end].unsqueeze(0),
            v[k_start:k_end].unsqueeze(0),
            out[q_start:q_end].unsqueeze(0),
            softmax_lse[:, q_start:q_end].unsqueeze(0).contiguous(),
            dq[q_start:q_end].unsqueeze(0),
            dk[k_start:k_end].unsqueeze(0),
            dv[k_start:k_end].unsqueeze(0),
            alibi_slopes_batch,
            dropout_p,
            softmax_scale,
            causal,
        ...
        softmax_d[:, q_start:q_end].copy_(softmax_d_batch[0])
...
def _flash_attn_varlen_backward(
    ...
    # dq, dk, dv are allocated by us so they should already be contiguous
    dout, q, k, v, out = [maybe_contiguous(x) for x in (dout, q, k, v, out)]
    if _needs_gfx11_varlen_d256_deterministic_fallback(q, deterministic):
        if dq is None or dk is None or dv is None:
            raise RuntimeError(
                "gfx11 D256 deterministic varlen backward fallback requires "
                "caller-provided dq, dk, and dv output tensors"
            )
        return _flash_attn_varlen_d256_deterministic_fallback(
            dout,
            q,
            k,
            v,
