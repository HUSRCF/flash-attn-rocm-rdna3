...
def is_gfx11_d64_bm128_s4096_enabled() -> bool:
    """Enable the validated D64 FP16 long-sequence tile unless explicitly rolled back."""
...
def is_gfx11_fwd_debug_tile_override_enabled() -> bool:
    ...
    debug_gates = (
        "CK_TILE_DEBUG_FWD_D64_BN64_FIRST",
        "CK_TILE_DEBUG_FWD_D128_BM64_BN64",
        "CK_TILE_DEBUG_FWD_D128_FORCE_BM128",
        "CK_TILE_DEBUG_FWD_D128_BM64_SHORT_S512",
        "CK_TILE_DEBUG_FWD_D128_BM64_SHORT_S256",
        "CK_TILE_DEBUG_FWD_D256_SHORT_BM16",
        "CK_TILE_DEBUG_FWD_D256_BM64_BN32",
        "CK_TILE_DEBUG_FWD_D256_BM64_BN128",
    )
    return any(os.getenv(gate, "0") == "1" for gate in debug_gates)


def get_gfx11_experimental_pipeline() -> str:
    """Select an opt-in pipeline used only by the narrow gfx11 D64 probe."""
    value = os.getenv("CK_TILE_DEBUG_FWD_GFX11_PIPELINE", "qr")
    pipeline_map = {
        "qr": "qr",
        "qs": "qs",
        "whole_k": "qr_whole_k",
    }
    if value not in pipeline_map:
        choices = ", ".join(pipeline_map)
        raise ValueError(
            f"CK_TILE_DEBUG_FWD_GFX11_PIPELINE must be one of: {choices}"
        )
    return pipeline_map[value]
...
// Native-O is on by default only for the per-instance validated whitelist.
// Defining this to 0 remains the exact safe-O rollback for precision A/B.
#ifndef CK_TILE_FMHA_FWD_WMMA_NATIVE_O_EPILOGUE
#define CK_TILE_FMHA_FWD_WMMA_NATIVE_O_EPILOGUE 1
#endif
"""
...
using fmha_pipeline_problem = ck_tile::BlockFmhaPipelineProblem<
    ...
    {F_trload},
    fmha_traits,
    CK_TILE_FMHA_FWD_WMMA_NATIVE_O_EPILOGUE && {F_native_o}>;

using fmha_pipeline = {F_pipeline}<
    fmha_pipeline_problem>;

// The pipeline problem owns the selection, so host and device compilation form
// the same kernel type while unrelated generated instances retain safe-O.
using fmha_epilogue = std::conditional_t<
    fmha_pipeline_problem::kUseNativeOEpilogue,
...
class FmhaFwdApiTrait:
    ...
    def scheck(self) -> str:
        ...
        if self.pipeline_tag in ["qr_async", "qr_async_trload", "qr_async_trload_v3"]:
            if self.spad == "t":
                ...
                return "true"
        elif self.pipeline_tag in ["qr", "qs", "qr_whole_k"]:
            if self.spad == "t":
    ...
    def seqtune(self, max_bm0: int) -> str:
        if (
            is_gfx11_d64_bm128_s4096_enabled()
            and self.arch.name == "gfx11"
            and str(self.hdim) == "64"
            and self.dtype == "fp16"
        ):
            is_long_sequence_target = (
                self.mode == "batch"
            ...
            if self.bm0 == 128:
                if is_long_sequence_target:
                    return "true/*gfx11 D64 FP16 S4096+ bm128/bn64 register-P*/"
                return "false/*gfx11 D64 bm128 is restricted to the validated target*/"
            if self.bm0 == 64:
                if is_long_sequence_target:
                    return "a.seqlen_q < 4096"
                return "true/*gfx11 D64 non-target bm64 fallback*/"
    ...
    def dcheck(self) -> str:
        if self.pipeline_tag == "qr_async":
            ...
            if self.dpad == "t":
                ...
                assert False
        elif self.pipeline_tag in ["qr", "qs", "qr_whole_k", "qr_async_trload", "qr_async_trload_v3"]:
...
class FmhaFwdKernel:
    ...
    @property
    def use_native_o_epilogue(self) -> bool:
        """Select native-O only for the runtime domains with precision coverage."""
        pipeline = self.F_pipeline
        common = (
            self.F_arch.name == "gfx11"
            and not is_gfx11_fwd_debug_tile_override_enabled()
            and pipeline.tag == "qr"
            and pipeline.F_vlayout == "row"
            and pipeline.F_logits == "f"
            and pipeline.F_mask in ("no", "s_no", "causal", "s_mask")
        ...
        equal_supported_dim = (
            self.F_hdim == self.F_tile.F_bn1 and self.F_hdim in (64, 128, 256)
        )
        # FP16 and BF16 both have strict safe/native precision coverage in this
        # exact batch domain. Other output types remain on the safe epilogue.
        native_dtype = self.F_dtype in ("fp16", "bf16")
        validated_batch = (
            self.F_mode == "batch"
            and native_dtype
            and equal_supported_dim
        )
        # Group/varlen remains on safe-O until it has the same strict bitwise
        # safe/native pair and profiler-dispatch evidence as batch mode.
    ...
    def render(self) -> str:
        return type(self)._KERNEL_HEADER + type(self)._KERNEL_BODY_TEMPLATE.format(
            ...
            F_native_o=BOOL_MAP[self.use_native_o_epilogue],
...
class KernelComponentFactoryGfx11(CompatibilityRuleFactory):
    ...
    def get_rules(cls) -> list[CompatibilityRule]:
        ...
        def check_d64_bm128_register_p(
            ...
            is_d64_fp16_bm128 = (
                problem_ctx.dtype == "fp16"
                and (problem_ctx.hdim, problem_ctx.hdim_v) == (64, 64)
                and kernel_ctx.tile.F_bm0 == 128
            )
            is_register_p_pipeline = (
                problem_ctx.dtype == "fp16"
                and (problem_ctx.hdim, problem_ctx.hdim_v) == (64, 64)
                and kernel_ctx.pipeline.tag == "qr"
                and kernel_ctx.pipeline.F_vlayout == "row"
                and kernel_ctx.pipeline.F_spad == "t"
                and kernel_ctx.pipeline.F_skpad == "t"
                ...
                and kernel_ctx.pipeline.F_bias == "no"
                and kernel_ctx.pipeline.F_lse == "t"
                and kernel_ctx.pipeline.F_dropout == "f"
                and kernel_ctx.pipeline.F_qscale == "no"
                and kernel_ctx.pipeline.F_skip == "f"
                and kernel_ctx.pipeline.F_trload == "f"
            )

            if is_d64_fp16_bm128:
                # bm128 remains restricted to the validated non-causal batch
                # target. bm64 is allowed to consume the same sequence-only
                # pipeline and remains the S<4096 dispatch fallback.
                return is_register_p_pipeline and problem_ctx.mode == "batch"
            return True

        rules.append(check_d64_bm128_register_p)

        def check_d64_whole_k_tile(
            problem_ctx: ProblemContext, kernel_ctx: KernelContext
        ) -> bool:
            is_d64 = (problem_ctx.hdim, problem_ctx.hdim_v) == (64, 64)
            is_whole_k_pipeline = kernel_ctx.pipeline.tag == "qr_whole_k"
            is_whole_k_tile = is_d64 and kernel_ctx.tile.F_bk1 == 16

            if is_whole_k_pipeline or is_whole_k_tile:
                # Whole-K requires N0/K1 >= 2. Pair its private K1=16
                # tiles only with that pipeline; all QR fallbacks retain K1=32.
                return is_whole_k_pipeline and is_whole_k_tile
            return True

        rules.append(check_d64_whole_k_tile)
        return rules
    ...
    def get_hdim_tile_size_dict(cls, dtype: str) -> Optional[dict]:
        if dtype in cls._DT_FP16_BF16:
            ...
            experimental_pipeline = get_gfx11_experimental_pipeline()
            ...
            if experimental_pipeline == "qr_whole_k":
                # Whole-K double-buffers V across the N0/K1 loop and requires
                # at least two iterations. Compatibility rules pair these
                # private tiles only with Whole-K, leaving QR fallbacks intact.
                if os.getenv("CK_TILE_DEBUG_FWD_D64_BN64_FIRST", "0") == "1":
                    raise ValueError(
                        "CK_TILE_DEBUG_FWD_GFX11_PIPELINE=whole_k conflicts with "
                        "CK_TILE_DEBUG_FWD_D64_BN64_FIRST=1"
                    )
                whole_k_tiles = [
                    FmhaFwdTileSize( 64,  32,  32,  64,  16,   64,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                    FmhaFwdTileSize( 64,  64,  32,  64,  16,   64,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                ]  # fmt: skip
                # API dispatch follows tile registration order. Put the exact
                # Whole-K candidates before the permissive full-D-pad QR
                # fallback so the experiment is reachable at runtime.
                tiles[(64, 64)] = whole_k_tiles + tiles[(64, 64)]
    ...
    def get_pipelines(
        ...
        if dtype in cls._DT_FP16_BF16:
            ...
            for logits, mask, bias, lse, dropout, skip in itertools.product(
                ...
                use_experimental_pipeline = (
                    experimental_pipeline != "qr"
                    and (hdim, hdim_v) == (64, 64)
                    and logits == "f"
                    and mask in ("no", "s_no")
                    and bias == "no"
                    and lse == "t"
                ...
                pipeline_tag = experimental_pipeline if use_experimental_pipeline else "qr"
                pipelines.append(FmhaFwdPipeline(pipeline_tag, "row", "f", "f", "f", "f", logits, bias, lse, dropout, qscale, mask, skip, "f"))  # fmt: skip
                if (
                    (hdim, hdim_v) in ((64, 64), (128, 128), (256, 256))
                    and logits == "f"
                    and mask in ("no", "s_no", "causal", "s_mask")
                    and bias == "no"
                    and lse == "t"
                    and dropout == "f"
                    and skip == "f"
                ):
                    # Keep sequence padding independent from D/DV padding. The
                    # latter stays false, so API dcheck/dvcheck restrict this
                    # native-O-capable instance to the exact validated head dim.
                    pipelines.append(FmhaFwdPipeline(pipeline_tag, "row", "t", "t", "f", "f", logits, bias, lse, dropout, qscale, mask, skip, "f"))  # fmt: skip
                # Keep head-dimension padding on QR: the D64 API bucket also
                # covers partial D33-D63, outside this exact-D64 experiment.
                pipelines.append(FmhaFwdPipeline("qr", "row", "t", "t", "t", "t", logits, bias, lse, dropout, qscale, mask, skip, "f"))  # fmt: skip
