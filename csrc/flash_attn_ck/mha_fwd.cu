                                       ...
                                       bool has_fp8_static_quant)
{
    return fmha_fwd_traits{head_size,
                           ...
                           false, // is_group_mode
                           true,  // is_v_rowmajor
                           softcap != 0.0f, // has_logits_soft_cap
                           mask.type,
                           enable_alibi ? bias_enum::alibi : bias_enum::no_bias,
                           has_lse,
                           has_dropout,
                           has_fp8_static_quant ? quant_scale_enum::pertensor : quant_scale_enum::no_scale}; // qscale_type
}
                                   ...
                                   std::pair<uint64_t*, uint64_t*> drop_seed_offset)
{
    ...
    return fmha_fwd_args{q.data_ptr(),
                         ...
                         q_descale_.has_value() ? q_descale_.value().data_ptr() : nullptr, // q_descale_ptr
                         k_descale_.has_value() ? k_descale_.value().data_ptr() : nullptr, // k_descale_ptr
                         v_descale_.has_value() ? v_descale_.value().data_ptr() : nullptr, // v_descale_ptr
        ...
        std::optional<at::Generator> gen_)
{
    ...
    if (seqlen_k > 0) {
        ...
        if(const char* ck_log_level = std::getenv("FLASH_ATTN_CK_LOG_LEVEL")) {
            stream_config.log_level_ = std::atoi(ck_log_level);
        }
